...
int Nodes,Links,Node_cost,OldPathLen;
int SrcID,DstID;
...
class SDN_switch: public node {
        ...
        bool one_hop_neighbors_exist; // you can use this variable to record the node's 1-hop neighbors
        unsigned int one_hop_neighbors;
...
class SDN_controller: public node {
        ...
        map<int,int> Next_Node_New;
        map<int,int> Next_Node_Old;
        map<int,int> Weight;
        map<int,int> visit;
        map<int,map<unsigned int,unsigned int>> nb_wight;
        map<int,int> old_node;
        map<int,int> now_node;
        map<int,int> recv;
        map<int,int> ctrl_recv_upd;
        //bool hi; // this is used for example; you can remove it when doing hw2
...
void SDN_controller::recv_handler (packet *p){
    if(p == nullptr) return;
    else if(p->type() == "SDN_ctrl_new_packet" && p->getHeader()->getSrcID() == getNodeID())
    {
        SDN_ctrl_new_packet *p2 = nullptr;
        p2 = dynamic_cast<SDN_ctrl_new_packet*> (p);
        SDN_ctrl_new_payload *l2 = nullptr;
        l2 = dynamic_cast<SDN_ctrl_new_payload*> (p2->getPayload());
        ctrl_recv++;
        unsigned mat = l2->getMatID();
        p2->getHeader()->setNexID( p2->getHeader()->getDstID() );
        if(ctrl_recv <= OldPathLen)
        {
            Next_Node_Old[p2->getHeader()->getDstID()] = l2->getActID();
            old_node[p2->getHeader()->getDstID()] = 1;
        }
        else
        {
            ctrl_recv_upd[p2->getHeader()->getDstID()]++;
        }
        send_handler (p2);
    }
    ...
    else if(p->type() == "SDN_ctrl_new_packet" && p->getHeader()->getSrcID() != getNodeID())
    {
        ...
        if(ctrl_recv > OldPathLen)
        {
            ctrl_recv_upd[p2->getHeader()->getSrcID()]++;
            for(int i = 0 ; i < Nodes ; i++)
            {
                if(ctrl_recv_upd[i] == 1)
                {
                    judge = 1;
                    break;
                }
            ...
            if(judge = 0)
            {
                for(int i = 0 ; i < Nodes ; i++)
                {
                    if(old_node[i] == 1 && now_node[i] == 1 && Next_Node_Old[i] != Next_Node_New[i] && i!=DstID)
                    {
                        ctrl_upd_packet_event(Nodes,i,DstID,Next_Node_New[i]);
                        judge = 1;
                    }
                }
                if(judge == 0)
                {
                    for(int i = 0 ; i < Nodes ; i++)
                    {
                        if(old_node[i] == 1 && now_node[i] == 0 && i!=DstID)
                        {
                            ctrl_del_packet_event(Nodes,i,DstID,Next_Node_Old[i]);
                        }
                    }
                }
            }
    ...
    else if(p->type() == "SDN_ctrl_upd_packet" && p->getHeader()->getSrcID() == getNodeID())
    {
    ...
    else if(p->type() == "SDN_ctrl_del_packet" && p->getHeader()->getSrcID() == getNodeID())
    {
        SDN_ctrl_del_packet *p2 = nullptr;
        p2 = dynamic_cast<SDN_ctrl_del_packet*> (p);
        SDN_ctrl_del_payload *l2 = nullptr;
        l2 = dynamic_cast<SDN_ctrl_del_payload*> (p2->getPayload());
        unsigned mat = l2->getMatID();
        p2->getHeader()->setNexID( p2->getHeader()->getDstID() );
        send_handler (p2);
    }
    ...
    else if(p->type() == "SDN_stat_packet")//收到SDN_stat_packet
    {
        ...
        Next_Node_New[p2->getHeader()->getSrcID()] = Next_Node_Old[p2->getHeader()->getSrcID()];
        stat_send[p2->getHeader()->getSrcID()] = 1;
        nb_wight[p2->getHeader()->getSrcID()] = l2->getLinkWeight();
        int update = 1;
        for(int i = 0 ; i < Nodes ; i++)
        {
            if(stat_send[i] != 1)
        ...
        if(update == 1)
        {
            Next_Node_New[DstID]=p2->getHeader()->getDstID();
            Weight[DstID]=0;
            //small_cost(DstID,DstID);
            for(int i = 0 ; i < Nodes ; i++)
            {
                Next_Node_New[i] = Next_Node_Old[i];
            }
            now_node[SrcID] = 1;
            now_node[DstID] = 1;
            old_node[DstID] = 1;
            int now = SrcID;
            while(Next_Node_New[now] != DstID)
            {
                now = Next_Node_New[now];
                now_node[now] = 1;
            }
            int judge = 0;
            for(int i = 0 ; i < Nodes ; i++)
            {
                if(old_node[i] == 0 && now_node[i] == 1 && i!=DstID)
                {
                    ctrl_new_packet_event(Nodes,i,DstID,Next_Node_New[i]);
                    judge = 1;
                }
            }
            if(judge == 0)
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        if(one_hop_neighbors == Nodes)// if one_hop_neighbors[p2->getHeader()->getDstID] doesn't have next node
        {
            return;
        }
        p2->getHeader()->setPreID ( getNodeID() );
        p2->getHeader()->setNexID ( one_hop_neighbors );
    ...
    else if(p->type() == "SDN_ctrl_new_packet")
    {
        ...
        one_hop_neighbors_exist=true;//let one_hop_neighbors[mat] exist
        one_hop_neighbors=act;//set the value of one_hop_neighbors[mat]
        p2->getHeader()->setDstID(p2->getHeader()->getSrcID());
        p2->getHeader()->setSrcID(getNodeID());
        p2->getHeader()->setPreID(getNodeID());
        p2->getHeader()->setNexID(p2->getHeader()->getDstID());
        send_handler(p2);
    }
    ...
    else if(p->type() == "SDN_ctrl_del_packet")
    {
        ...
        unsigned act = l2->getActID();
        // string msg = l3->getMsg(); // get the msg
        one_hop_neighbors_exist=false;//let one_hop_neighbors[mat] exist
        one_hop_neighbors = Nodes;
        p2->getHeader()->setDstID(p2->getHeader()->getSrcID());
        p2->getHeader()->setSrcID(getNodeID());
        p2->getHeader()->setPreID(getNodeID());
        p2->getHeader()->setNexID(p2->getHeader()->getDstID());
...
int main()
{
    int InsTime,UpdTime,SimDuration,DoTime;
    int OldPathLinkID,Node1,Node2,LinkID,newW;
    ...
    map<int,map<unsigned int, unsigned int>> nb_weight;
    ...
    for(int i=0;i<Links;i++)
    {
        ...
        nb_weight[Node1][Node2] = newW;
        nb_weight[Node2][Node1] = newW;
    }
    for(int i=0;i<Nodes;i++)
    {
    ...
    data_packet_event(src, dst, DoTime);
    ...
    // for (int id = node::getNodeNum()-1; id >= 0; id --){ // this line is used to check whether the sequence affects the results
        ...
        link_weight = nb_weight[id];
        stat_packet_event(id, con_id, link_weight, UpdTime);
    }
    // start simulation!!
...
void SDN_controller::small_cost(int Dst ,int node)
{
    visit[node] = 1;
    for(int i =0 ; i< Nodes ; i++)
    {
        if(nb_wight[node][i] != 0  && i != Dst)
        {
            if(Next_Node_Old[i] == node && old_node[i] == 1 && old_node[node] == 1)
            {
                if(Weight[i] != 0 && Weight[i] > nb_wight[node][i] + Weight[node] )
                {
                    Next_Node_New[i] = node;
                    Weight[i] = nb_wight[node][i] + Weight[node];
                }
                else if(Weight[i] == 0)
                {
                    Next_Node_New[i] = node;
            ...
            else if(Next_Node_Old[i] != node && old_node[i] == 1 && old_node[node] == 1)
            {
                int time = 0,temp = i;
                while(temp != node)
                {
                    temp = Next_Node_Old[temp];
                    time++;
                    if(temp == Dst)
                    {
                        time = -1;
                ...
                if(time == -1)
                {
                    if(Weight[i] != 0 && Weight[i] > nb_wight[node][i] + Weight[node] + Node_cost)
                    {
                        Next_Node_New[i] = node;
                        Weight[i] = nb_wight[node][i] + Weight[node] + Node_cost;
                    }
                    else if(Weight[i] == 0)
                    {
                        Next_Node_New[i] = node;
                ...
                else if(Weight[i] != 0 && Weight[i] > nb_wight[node][i] + Weight[node] + time*Node_cost)
                {
                    Next_Node_New[i] = node;
                    Weight[i] = nb_wight[node][i] + Weight[node] + time*Node_cost;
                }
                else if(Weight[i] == 0)
                {
                    Next_Node_New[i] = node;
            ...
            else if( old_node[i] == 0 && old_node[node] == 1)
            {
                if(Weight[i] != 0 && Weight[i] > nb_wight[node][i] + Weight[node] + Node_cost)
                {
                    Next_Node_New[i] = node;
            ...
            else if( old_node[i] == 1 && old_node[node] == 0)
            {
                int time = 0,j;
                j = node;
                while(old_node[j] != 1)
            ...
            else if( old_node[i] == 0 && old_node[node] == 0)
            {
                if(Weight[i] != 0 && Weight[i] > nb_wight[node][i] + Weight[node] + Node_cost)
                {
                    Next_Node_New[i] = node;
    ...
    for(int i = 0 ; i < Nodes ; i++)
    {
        if(Weight[i] != 0 && visit[i] != 1 && tmp == -1)
        {
            tmp = i;
        }
        else if(Weight[i] != 0 && visit[i] != 1)
        {
            if(Weight[tmp] > Weight[i])
            {
                tmp = i;
            }
        }
