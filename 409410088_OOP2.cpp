...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        string msg_dst,msg_Next_Node;
        int a=0;
        while(msg[a] !=' ')
        {
            msg_dst=msg_dst+msg[a];
            a++;
        }
        a++;
        while(msg[a]!='\0')
        {
            msg_Next_Node=msg_Next_Node+msg[a];
            a++;
        }
        one_hop_neighbors_exist[stoi(msg_dst)]=true;//let one_hop_neighbors[stoi(msg_dst)] exist
        one_hop_neighbors[stoi(msg_dst)]=stoi(msg_Next_Node);//set the value of one_hop_neighbors[stoi(msg_dst)]
        //cout << getNodeID() << " received packet with msg context \"" << stoi(msg_dst) <<stoi(msg_Next_Node)<<"\""<< endl;
...
class data_event //save data_packet_event
    {
    public:
        int t;
        int src;
...
class data_event de[5000];

class control_event // save control_packet_event
    {
    public:
        int id;
        string msg;
    };
class control_event ce[5000];

int cmp(data_event const & c ,data_event const & d ) 
{ 
    if(c.t != d.t) 
    {
        return c.t < d.t; 
...
int cmp_control(control_event const & c ,control_event const & d ) 
{ 
    if(c.id != d.id) 
    {
        return c.id < d.id; 
...
int main()
{
    ...
    while(cin>>t>>src>>dst)
    { 
        de[num_of_data_packet].t=t;
        de[num_of_data_packet].src=src;
        de[num_of_data_packet].dst=dst;
        num_of_data_packet++;
    }
    sort(de,de+num_of_data_packet,cmp);//sort the data_packet_events
    int b=0;
    for(int i=0;i<num_of_data_packet;i++)
    {
        if(de[i].t>=InsTime)
        {
            b=i;
        ...
        data_packet_event(de[i].src,de[i].dst,de[i].t);
    }
    //cout<<b<<endl;
    int num_of_control_packet=0;
    for(unsigned int id = 0; id < node::getNodeNum(); id++)
    {
        for(int i=0;i<Dsts;i++)
        {
            if(id != Dst[i])
            {
                string msg =to_string(Dst[i])+" "+to_string(Next_Node_Old[id][Dst[i]]);
                ce[num_of_control_packet].id=id;
                ce[num_of_control_packet].msg=msg;
                num_of_control_packet++;
            }
        }
    }    
    sort(ce,ce+num_of_control_packet,cmp_control);//sort control_packet_events in InsTime
    for(int i=0;i<num_of_control_packet;i++)
    {
        ctrl_packet_event(ce[i].id, InsTime, ce[i].msg);
    }
    for(int i=b;i<num_of_data_packet;i++)
    {
        if(de[i].t>=UpdTime)
        {
            b=i;
    ...
    //cout<<b<<endl;
    num_of_control_packet=0;
    for(unsigned int id = 0; id < node::getNodeNum(); id++)
    {
        for(int i=0;i<Dsts;i++)
        {
            if( Next_Node_Old[id][Dst[i]]!= Next_Node_New[id][Dst[i]])
            {
                string msg =to_string(Dst[i])+" "+to_string( Next_Node_New[id][Dst[i]]);
                //cout<<id<<" "<<msg<<" "<<Next_Node_Old[id][Dst[i]] <<endl;
                ce[num_of_control_packet].id=id;
                ce[num_of_control_packet].msg=msg;
                num_of_control_packet++;
    ...
    sort(ce,ce+num_of_control_packet,cmp_control);//sort control_packet_events in UpdTime
    for(int i=0;i<num_of_control_packet;i++)
    {
        ctrl_packet_event(ce[i].id, UpdTime, ce[i].msg);
    }
    for(int i=b;i<num_of_data_packet;i++)
    {
        data_packet_event(de[i].src,de[i].dst,de[i].t);
