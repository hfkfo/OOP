...
static map<int,map<int,int>> Next_Node_New;
static map<int,map<int,int>> Next_Node_Old;
static map<int,map<int,int>> Weight;
static map<int,map<int,int>> visit;
...
void shortest_path(int Node,int Dst,int old_or_new)
{
    const map<unsigned int,bool> &nblist = node::id_to_node(Node)->getPhyNeighbors();
    visit[Node][Dst]=1;
    ...
    {    
        ...
        if(tmp != Nodes)
        {
            for(int i=0;i<Links;i++)
            {
                ...
                else if(Link[i].Node1==tmp)
                {
                    if(Link[i].Node2==Node)
                    {
                        tmp=i;
                        break;
                    }
            ...
            if(old_or_new==0)
            {
                tmp1=Weight[Node][Dst]+Link[tmp].oldW; 
            }
            else if(old_or_new==1)
            {
                tmp1=Weight[Node][Dst]+Link[tmp].newW;
            }
            if(Link[tmp].Node1!=Node)
            {
                if(Weight[Link[tmp].Node1][Dst]==0)
                {
                    if(old_or_new==0)
                    {
                        if(Link[tmp].Node1!=Dst)
                        {
                            Weight[Link[tmp].Node1][Dst]=tmp1;
                            Next_Node_Old[Link[tmp].Node1][Dst]=Node;
                        }
                    }
                    else
                    {
                        ...
                        {
                            Weight[Link[tmp].Node1][Dst]=tmp1;
                            Next_Node_New[Link[tmp].Node1][Dst]=Node;
                        }
                    }
                }
                else if(Weight[Link[tmp].Node1][Dst]>tmp1)
                {
                    Weight[Link[tmp].Node1][Dst]=tmp1;
                    if(old_or_new==0)
                    {
                ...
                else if(Weight[Link[tmp].Node1][Dst]==tmp1)
                {
                    if(old_or_new==0)
                    {
                        if(Next_Node_Old[Link[tmp].Node1][Dst] > Node)
                        {
                            Weight[Link[tmp].Node1][Dst]=tmp1;
                            Next_Node_Old[Link[tmp].Node1][Dst]=Node;
                        }
                    ...
                    {
                        if(Next_Node_New[Link[tmp].Node1][Dst] > Node)
                        {
                            Weight[Link[tmp].Node1][Dst]=tmp1;
                            Next_Node_New[Link[tmp].Node1][Dst]=Node;
                        }
            ...
            {
                if(Weight[Link[tmp].Node2][Dst]==0)
                {
                    if(old_or_new==0)
                    {
                        if(Link[tmp].Node2!=Dst)
                        {
                            Weight[Link[tmp].Node2][Dst]=tmp1;
                            Next_Node_Old[Link[tmp].Node2][Dst]=Node;
                        }
                    }
                    else
                    {
                        ...
                        {
                            Weight[Link[tmp].Node2][Dst]=tmp1;
                            Next_Node_New[Link[tmp].Node2][Dst]=Node;
                        }
                    }
                }
                else if(Weight[Link[tmp].Node2][Dst]>tmp1)
                {
                    Weight[Link[tmp].Node2][Dst]=tmp1;
                    if(old_or_new==0)
                    {
                ...
                else if(Weight[Link[tmp].Node2][Dst]==tmp1)
                {
                    if(old_or_new==0)
                    {
                        if(Next_Node_Old[Link[tmp].Node2][Dst] > Node)
                        {
                            Weight[Link[tmp].Node2][Dst]=tmp1;
                            Next_Node_Old[Link[tmp].Node2][Dst]=Node;
                        }
                    ...
                    {
                        if(Next_Node_New[Link[tmp].Node2][Dst] > Node)
                        {
                            Weight[Link[tmp].Node2][Dst]=tmp1;
                            Next_Node_New[Link[tmp].Node2][Dst]=Node;
                        }
...
void do_shortest_path()
{
    ...
    {
        Next_Node_Old[Dst[i]][Dst[i]]=Nodes;  
        Weight[Dst[i]][Dst[i]]=0;
        shortest_path(Dst[i],Dst[i],0);
        Weight.clear();
        visit.clear();
    } 
    for(int i=0;i<Dsts;i++)
    {
        Next_Node_New[Dst[i]][Dst[i]]=Nodes;
        Weight[Dst[i]][Dst[i]]=0;
        shortest_path(Dst[i],Dst[i],1);
        Weight.clear();
        visit.clear();
    }
}
void SDN_controller::recv_handler (packet *p){
    ...
    else if(p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() == getNodeID())
    {
        ...
        if(stop == 0 && ctrl_recv[mat][p2->getHeader()->getPreID()] == 4)// judge if it is initial or update
        {
            int cur_level;
            bool change = 0;
            cur_level = level_new[p2->getHeader()->getPreID()][mat] + 1;
            while(cur_level <= max_level[mat])//find the minimum level of changed nodes
            {
                for(int id=0;id<Nodes;id++)
                {
                    if(level_new[id][mat] == cur_level && Next_Node_New[id][mat] != Next_Node_Old[id][mat])
                    {
                        change = 1;
...
int main()
{
    ...
    // dst = 0;
    int cur_level = 1;
    map<int,map<int,int>>if_have_parent;
    map<int,map<int,int>>if_next_node_change;
    for(int i=0;i<Dsts;i++)//set first next node
    {   
        {
            for(int id=0;id<Nodes;id++)
            {
                if(id != Dst[i])
                {
                    if_have_parent[Next_Node_Old[id][Dst[i]]][Dst[i]] = 1 ;
                    ctrl_packet_event(con_id, id, Dst[i], Next_Node_Old[id][Dst[i]], InsTime);
                }
                if(Next_Node_Old[id][Dst[i]] == Next_Node_New[id][Dst[i]] && id != Dst[i])
                {
                    if_next_node_change[id][Dst[i]] = 1;
                }
            }
        }
    }
    set_level_new();
    bool change = 0;
    for(int i=0;i<Dsts;i++)//set first update next node
    {
        change = 0;
        cur_level = 1;
        while(cur_level <= max_level[Dst[i]])//find the minimum level of changed nodes
        {
            for(int id=0;id<Nodes;id++)
            {
                if(level_new[id][Dst[i]] == cur_level && Next_Node_New[id][Dst[i]] != Next_Node_Old[id][Dst[i]])
                {
                    change = 1;
                    ctrl_packet_event(con_id, id, Dst[i], Next_Node_New[id][Dst[i]], UpdTime);
                }
            }
            if(change == 1)
            {
        ...
        for(int id = 0 ;id<Nodes;id++) 
        {
            if(level_new[id][Dst[i]] > cur_level  && Next_Node_New[id][Dst[i]] != Next_Node_Old[id][Dst[i]])//if no other node connect to this node
            {
                if(if_have_parent[id][Dst[i]] == 0 )
                {
                    level_new[id][Dst[i]] = cur_level;
                    ctrl_packet_event(con_id, id, Dst[i], Next_Node_New[id][Dst[i]], UpdTime);
                }
                else if(if_next_node_change[Next_Node_New[id][Dst[i]]][Dst[i]] == 1)//if next new node don't change next node
                {
                    level_new[id][Dst[i]] = cur_level;
                    ctrl_packet_event(con_id, id, Dst[i], Next_Node_New[id][Dst[i]], UpdTime);
                }
...
void set_level_new() // set the new weight level
{
    int num = 0;
    int cur_level = 0;
    for(int i=0;i<Dsts;i++)
    {
        for(int id = 0;id < Nodes ; id++)
        {
            ...
            {
                level_new[id][Dst[i]] = cur_level + 1;
                visit[id][Dst[i]] = 1;
                num++;
            }
        }
        ...
        while(num < Nodes - 1)
        {
            for(int id = 0; id < Nodes ; id++)
            {
                if(visit[id][Dst[i]] == 0 && level_new[Next_Node_New[id][Dst[i]]][Dst[i]] == cur_level)
                {
                    level_new[id][Dst[i]] = cur_level + 1;
                    visit[id][Dst[i]] = 1;
                    num++;
