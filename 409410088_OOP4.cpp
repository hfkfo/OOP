...
class SDN_stat_payload : public payload {
    ...
    public:
        ...
        //SET(setActID,unsigned int,actID,_actID);
        //GET(getActID,unsigned int,actID);
        //SET(setInfo, vector<pair<unsigned int,unsigned int>> ,stat_info,_stat_info);
        //GET(getInfo, vector<pair<unsigned int,unsigned int>> ,stat_info);
        void setInfo(vector<pair<unsigned int,unsigned int>> _stat_info)
        {
            stat_info = _stat_info;
        }
        vector<pair<unsigned int,unsigned int>> getInfo()const
        {
            return stat_info;
        }
...
class SDN_switch: public node {
        map<unsigned int,bool> one_hop_neighbors_exist; // you can use this variable to record the node's 1-hop neighbors
        map<unsigned int,int> one_hop_neighbors;
...
class SDN_controller: public node {
        ...
        map<int,map<int,int>>ctrl_recv;//the time controller recv
        map<int,int> stat_send;
        map<int,map<int,int>> Next_Node_New;
        map<int,map<int,int>> Next_Node_Old;
        map<int,map<int,int>> Weight;
        map<int,map<int,int>> visit;
        map<int,vector<pair<unsigned int,unsigned int>>> nb_wight;
        map<int,int> max_level;
    ...
    public:
        ...
        void shortest_path_old(int,int);
        void shortest_path_new(int,int);
        void set_level();
        // please define recv_handler function to deal with the incoming packet
        virtual void recv_handler (packet *p);
...
void SDN_controller::shortest_path_old(int Node,int Dst)
{
    visit[Node][Dst]=1;
    for(vector<pair<unsigned int,unsigned int>>::const_iterator it = nb_wight[Node].begin(); it != nb_wight[Node].end(); it ++)
    {
        if(Weight[it->first][Dst]==0)
        {
            if(it->first!=Dst)
            {
                Weight[it->first][Dst]=Weight[Node][Dst]+it->second;
                Next_Node_Old[it->first][Dst]=Node;
            }
        }
        else if(Weight[it->first][Dst]> Weight[Node][Dst]+it->second)
        {
            Weight[it->first][Dst]= Weight[Node][Dst]+it->second;
            Next_Node_Old[it->first][Dst]=Node;
        }
        ...
        {
            if(Next_Node_Old[it->first][Dst] > Node)
            {
                    Weight[it->first][Dst]= Weight[Node][Dst]+it->second;
                    Next_Node_Old[it->first][Dst]=Node;
            }
    ...
    {
        if(visit[i][Dst]==0 && Weight[i][Dst]!=0 && tmp==-1)
        {
            tmp=i;
        }
        else if(visit[i][Dst]==0 && Weight[i][Dst]!=0)
        {
            if(Weight[i][Dst]<Weight[tmp][Dst])
            {
                tmp=i;
            }
        }
        else if(Weight[i][Dst]==0)
        {
             Next_Node_Old[i][Dst]=Nodes;
        }
    }

    if(tmp!=-1)
    {
        shortest_path_old(tmp,Dst);
    }
}

void SDN_controller::shortest_path_new(int Node,int Dst)
{
    visit[Node][Dst]=1;
    for(vector<pair<unsigned int,unsigned int>>::const_iterator it = nb_wight[Node].begin(); it != nb_wight[Node].end(); it ++)
    {
        if(Weight[it->first][Dst]==0)
        {
            ...
            {
                Weight[it->first][Dst]=Weight[Node][Dst]+it->second;
                Next_Node_New[it->first][Dst]=Node;
            }
        }
        else if(Weight[it->first][Dst]> Weight[Node][Dst]+it->second)
        {
        ...
        {
            if(Next_Node_New[it->first][Dst] > Node)
            {
                    Weight[it->first][Dst]= Weight[Node][Dst]+it->second;
                    Next_Node_New[it->first][Dst]=Node;
            }
    ...
    for(int i=0;i<Nodes;i++)
    {
        ...
        {
             Next_Node_New[i][Dst]=Nodes;
        }
    }

    if(tmp!=-1)
    {
        shortest_path_new(tmp,Dst);
    }
}
...
void SDN_controller::recv_handler (packet *p){
    ...
    else if(p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() == getNodeID())
    {
        ...
        ctrl_recv[mat][p2->getHeader()->getPreID()]++; // judge the time of controller receive
        bool stop = 0;
        for(int i=0;i<Nodes;i++)//jugde if all send control packet are received
        {
            if(ctrl_recv[mat][i] == 3)
            {
                stop = 1;
                break;
            }
        }
        if(stop == 0 && ctrl_recv[mat][p2->getHeader()->getPreID()] == 4)// judge if it is initial or update
        {
            int cur_level;
            bool change = 0;
            cur_level = level[p2->getHeader()->getPreID()][mat] + 1;
            while(cur_level <= max_level[mat])//find the minimum level of changed nodes
            {
                for(int id=0;id<Nodes;id++)
                {
                    if(level[id][mat] == cur_level && Next_Node_New[id][mat] != Next_Node_Old[id][mat])
                    {
                        change = 1;
                        ctrl_packet_event(Nodes, id, mat, Next_Node_New[id][mat]);
                    }
                }
                if(change == 1)
                {
    ...
    else if(p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() != getNodeID())
    {
        ...
        SDN_ctrl_payload *l2 = nullptr;
        l2 = dynamic_cast<SDN_ctrl_payload*> (p2->getPayload());
        ctrl_recv[l2->getMatID()][p2->getHeader()->getDstID()]++; // jugde the time of controller receive
        p2->getHeader()->setPreID ( getNodeID() );
        p2->getHeader()->setNexID ( p2->getHeader()->getDstID() );
        p2->getHeader()->setDstID ( p2->getHeader()->getDstID() );
        send_handler (p2);
    }
    else if(p->type() == "SDN_stat_packet")//收到SDN_stat_packet
    {
        SDN_stat_packet *p2 = nullptr;
        p2 = dynamic_cast<SDN_stat_packet*> (p);
        SDN_stat_payload *l2 = nullptr;
        l2 = dynamic_cast<SDN_stat_payload*> (p2->getPayload());
        stat_send[p2->getHeader()->getSrcID()]++;
        nb_wight[p2->getHeader()->getSrcID()] = l2->getInfo();
        int install = 0;
        int update = 0;
        for(int i=0; i < p2->getHeader()->getDstID() ; i++)//判斷是install或update
        {
            if(stat_send[i] == 0)
            {
                break;
            }
            else if(stat_send[i] == 1)
            {
                install++;
                if(update > 0)
                {
        ...
        if(install == p2->getHeader()->getDstID())//if install
        {
            for(int i=0;i<Dsts;i++)
            {
                Next_Node_Old[Dst[i]][Dst[i]]=p2->getHeader()->getDstID();
                Weight[Dst[i]][Dst[i]]=0;
                shortest_path_old(Dst[i],Dst[i]);//build rounting table
                Weight.clear();
                visit.clear();
                for(int id=0;id<Nodes;id++)
                {
                    if(id != Dst[i])
                    {
                        ctrl_packet_event(p2->getHeader()->getDstID(), id, Dst[i], Next_Node_Old[id][Dst[i]]);
                    }
                }
            }
        }
        else if(update == p2->getHeader()->getDstID())
        {
            for(int i=0;i<Dsts;i++)
            {
                Next_Node_New[Dst[i]][Dst[i]]=p2->getHeader()->getDstID();
                Weight[Dst[i]][Dst[i]]=0;
                shortest_path_new(Dst[i],Dst[i]);//build rounting table
                Weight.clear();
                visit.clear();
                int cur_level = 1;
                int change = 0;
                set_level();
                while(cur_level <= max_level[Dst[i]])//find the minimum level of changed nodes
                {
                    for(int id=0;id<Nodes;id++)
                    {
                        if(level[id][Dst[i]] == cur_level && Next_Node_New[id][Dst[i]] != Next_Node_Old[id][Dst[i]])
                        {
                            change = 1;
                            ctrl_packet_event(p2->getHeader()->getDstID(), id, Dst[i], Next_Node_New[id][Dst[i]]);
                        }
                    }
                    if(change == 1)
                    {
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        //cout<<getNodeID()<<" "<<one_hop_neighbors[p2->getHeader()->getDstID()]<<" "<<p2->getHeader()->getDstID()<<endl;
        if(!one_hop_neighbors_exist[p2->getHeader()->getDstID()])//if one_hop_neighbors[p2->getHeader()->getDstID] doesn't exist
        {
            return;
        }
        if(one_hop_neighbors[p2->getHeader()->getDstID()] == Nodes)// if one_hop_neighbors[p2->getHeader()->getDstID] doesn't have next node
        {
            return;
        }
        p2->getHeader()->setPreID ( getNodeID() );
        p2->getHeader()->setNexID ( one_hop_neighbors[p2->getHeader()->getDstID()] );
        p2->getHeader()->setDstID ( p2->getHeader()->getDstID() );
        send_handler (p2);
    }
        ...
        one_hop_neighbors_exist[mat]=true;//let one_hop_neighbors[mat] exist
        one_hop_neighbors[mat]=act;//set the value of one_hop_neighbors[mat]
        p3->getHeader()->setPreID ( getNodeID() );
        p3->getHeader()->setNexID ( p3->getHeader()->getSrcID() );
        p3->getHeader()->setDstID ( p3->getHeader()->getSrcID() );
        send_handler (p3);
    }
    else if(p->type() == "SDN_stat_packet")//將node的鄰邊送到controller
    {
...
int main()
{
    ...
    for(int i=0;i<Links;i++)
    {
        ...
        node::id_to_node(Link[i].Node1)->add_phy_neighbor(Link[i].Node2);
        node::id_to_node(Link[i].Node2)->add_phy_neighbor(Link[i].Node1);
    }
    for(int i=0;i<Nodes;i++)
    {
    ...
    for(int i=0;i<Nodes;i++)
    {
        vector<pair<unsigned int,unsigned int>> stat_info;//儲存node的鄰邊
        ...
        for(map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++)
        {
            ...
            {
                if(Link[j].Node1== i || Link[j].Node1== it->first)
                {
                    if(Link[j].Node2== it->first || Link[j].Node2== i)
                    {
                        weight = Link[j].oldW;
                        break;
                    }
            ...
            stat_info.push_back(make_pair(it->first,weight));
        }
        stat_packet_event(con_id,i,stat_info,InsTime);
    }
...
void SDN_controller::set_level() // set the update level
{
    int num = 0;
    int cur_level = 0;
    for(int i=0;i<Dsts;i++)
    {
        for(int id = 0;id < Nodes ; id++)
        {
            if(visit[id][Dst[i]] == 0 && Next_Node_New[id][Dst[i]] == Dst[i] && id != Dst[i])
            {
                level[id][Dst[i]] = cur_level + 1;
                visit[id][Dst[i]] = 1;
                num++;
            }
        }
        ...
        while(num < Nodes - 1)
        {
            for(int id = 0; id < Nodes ; id++)
            {
                if(visit[id][Dst[i]] == 0 && level[Next_Node_New[id][Dst[i]]][Dst[i]] == cur_level)
                {
                    level[id][Dst[i]] = cur_level + 1;
                    visit[id][Dst[i]] = 1;
                    num++;
